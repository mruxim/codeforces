...
int mincom[2*maxn][maxn][maxn];
int com[2*maxn][maxn][maxn], st[2*maxn][maxn][maxn], ed[2*maxn][maxn][maxn];
...
//		v2[y][x] = v1[x][y]; reverse (all (v2[y][x]));
		copy (rall(v1[x][y]), back_inserter (v2[y][x]));
	}
	...
	rep (i, n) rep (j, n) if (g1[i][j])
	{
		rep (k, sz(v1[i][j])-1) if (v1[i][j][k] == i && v1[i][j][k+1] == j)
		{
			...
			int v = i, u = j;
			if (sz(prv)) v = prv.back();
			if (sz(nxt)) u = nxt.back();

			mincom[sz(prv) + 2 + sz(nxt)][v][u]++;
		}
	}
	cout << 0 << endl;	
